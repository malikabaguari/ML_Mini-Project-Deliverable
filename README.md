...
This project develops a supervised Machine Learning model to predict the diesel price at French fuel stations. The flagship algorithm is XGBoost Regressor, as required by the project brief. A lightly configured Random Forest is included as a reference baseline model.
...
- checks for exact duplicate rows and duplicate station-fuel-date combinations, and removes isolated inconsistent records after inspecting their temporal and cross-sectional context (for example, a single conflicting E10 observation and an isolated EUR 3.00/L diesel outlier);
...
- diesel price differences by department (roughly EUR 0.20/L spread between the cheapest and most expensive areas).
...
The selected configuration is `n_estimators = 1200`, `learning_rate = 0.1`, `subsample = 0.9`, `colsample_bytree = 0.6`, `reg_lambda = 3`, reaching a validation MAE of about 0.0374 EUR/L on October.
...
- Random Forest baseline (October validation): MAE 0.0487 EUR/L, RMSE 0.0614 EUR/L, R² 0.509;
- Initial XGBoost (October validation): MAE 0.0380 EUR/L, RMSE 0.0502 EUR/L, R² 0.672;
- Final tuned XGBoost (November-December test): MAE 0.0507 EUR/L, RMSE 0.0639 EUR/L, R² 0.596;
- Final tuned XGBoost (training set, for comparison): MAE 0.0303 EUR/L, R² 0.835.

The gap between training and test performance points to some overfitting, but the test period also shows a genuine shift in diesel prices - the monthly median rose from EUR 1.602/L in October to EUR 1.679/L in November before falling back to EUR 1.599/L in December - which also limits achievable test performance.
...
| Modeling notebook | `ML_Project_Diesel_Prices.ipynb` |
| Presentation | `ML_Project_Diesel_Prices_Presentation.pptx` |
| Raw price data | `data/raw/prices_2025.csv` |
| Raw station data | `data/raw/stations_2025.csv` |
| Raw Brent crude oil data | `data/raw/RBRTEd.xls` |
| Raw GPR data | `data/raw/data_gpr_daily_recent.xls` |

The Google Slides presentation should contain the 11-slide structure required by the brief: title, project overview, two data-preparation and feature-engineering slides, three model-building and tuning slides, two key-findings slides, future work, and closing.

Add the presentation link here once the final Google Slides version is ready.
...
```text
ML_Project_Diesel_Prices.ipynb
```
